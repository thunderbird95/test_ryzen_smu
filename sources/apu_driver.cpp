...
EXP ryzen_access ApuDriver::init_ryzenadj()
{
    ryzen_access ry;
    enum ryzen_family family = smu.cpuid_get_family();
    ...
    ry->pci_obj = smu.init_pci_obj();
    if(!ry->pci_obj){
        printf("Unable to get PCI Obj, check permission\n");
        return NULL;
    }

    ry->nb = smu.get_nb(ry->pci_obj);
    if(!ry->nb){
        printf("Unable to get NB Obj\n");
        goto out_free_pci_obj;
    }

    ry->mp1_smu = smu.get_smu(ry->nb, TYPE_MP1);
    if(!ry->mp1_smu){
        printf("Unable to get MP1 SMU Obj\n");
        goto out_free_nb;
    }

    ry->psmu = smu.get_smu(ry->nb, TYPE_PSMU);
    if(!ry->psmu){
...
out_free_mp1_smu:
    smu.free_smu(ry->mp1_smu);
out_free_nb:
    smu.free_nb(ry->nb);
out_free_pci_obj:
    smu.free_pci_obj(ry->pci_obj);
    free(ry);
    return NULL;
}

EXP void ApuDriver::cleanup_ryzenadj(ryzen_access ry){
    if (ry == NULL)
        return;
    ...
    if (ry->mem_obj){
        smu.free_mem_obj(ry->mem_obj);
    }
    smu.free_smu(ry->psmu);
    smu.free_smu(ry->mp1_smu);
    smu.free_nb(ry->nb);
    smu.free_pci_obj(ry->pci_obj);
    free(ry);
...
EXP enum ryzen_family ApuDriver::get_cpu_family(ryzen_access ry)
{
    return ry->family;
}

EXP int ApuDriver::get_bios_if_ver(ryzen_access ry)
{
    if(ry->bios_if_ver)
        return ry->bios_if_ver;

    smu_service_args_t args = {0, 0, 0, 0, 0, 0};
    smu.smu_service_req(ry->mp1_smu, 0x3, &args);
...
int ApuDriver::request_table_ver_and_size(ryzen_access ry)
{
    ...
    smu_service_args_t args = {0, 0, 0, 0, 0, 0};
    resp = smu.smu_service_req(ry->psmu, get_table_ver_msg, &args);
...
int ApuDriver::request_table_addr(ryzen_access ry)
{
    ...
    resp = smu.smu_service_req(ry->psmu, get_table_addr_msg, &args);
...
int ApuDriver::request_transfer_table(ryzen_access ry)
{
    int resp;
    ...
    resp = smu.smu_service_req(ry->psmu, transfer_table_msg, &args);
...
EXP int ApuDriver::init_table(ryzen_access ry)
{
    DBG("init_table\n");
    int errorcode = 0;
    ...
    ry->mem_obj = smu.init_mem_obj(ry->table_addr);
    if(!ry->mem_obj)
    {
    ...
    ry->table_values = (float*)(calloc(ry->table_size / 4, 4));
...
EXP uint32_t ApuDriver::get_table_ver(ryzen_access ry)
{
    int errorcode;
    _lazy_init_table(0);
...
EXP size_t ApuDriver::get_table_size(ryzen_access ry)
{
    int errorcode;
    _lazy_init_table(0);
...
EXP float* ApuDriver::get_table_values(ryzen_access ry)
{
    return ry->table_values;
}

EXP int ApuDriver::refresh_table(ryzen_access ry)
{
    int errorcode = 0;
    _lazy_init_table(errorcode);

    //only execute request table if we don't use SMU driver
    if(!smu.is_using_smu_driver()){
        ...
        if(ry->table_values[0] == 0 || smu.compare_pm_table(ry->table_values, 6 * 4) == 0){
    ...
    if(smu.copy_pm_table(ry->table_values, ry->table_size)){
...
int ApuDriver::setValueToSmu(ryzen_access ry, bool ispSmu, uint32_t address, uint32_t value)
{
    if (!ispSmu)
        _do_adjust(address);
    if (ispSmu)
        _do_adjust_psmu(address);
    return ADJ_ERR_FAM_UNSUPPORTED;
}

int ApuDriver::setValueToSmu(ryzen_access ry, bool ispSmu, uint32_t address, uint32_t value0, uint32_t value1)
{
    if (!ispSmu)
        _do_adjust2(address);
    if (ispSmu)
...
EXP int ApuDriver::set_stapm_limit(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_fast_limit(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_slow_limit(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_slow_time(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_stapm_time(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_tctl_temp(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_vrm_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_vrmsoc_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_vrmgfx_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_VANGOGH:
        _do_adjust(0x1c);
...
EXP int ApuDriver::set_vrmcvip_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_VANGOGH:
        _do_adjust(0x1d);
...
EXP int ApuDriver::set_vrmmax_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_vrmgfxmax_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_VANGOGH:
        _do_adjust(0x1f);
...
EXP int ApuDriver::set_vrmsocmax_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_psi0_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_psi3cpu_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_VANGOGH:
        _do_adjust(0x20);
...
EXP int ApuDriver::set_psi0soc_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_psi3gfx_current(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_VANGOGH:
        _do_adjust(0x21);
...
EXP int ApuDriver::set_max_gfxclk_freq(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_min_gfxclk_freq(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_max_socclk_freq(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_min_socclk_freq(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_max_fclk_freq(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_min_fclk_freq(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_max_vcn(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_min_vcn(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_max_lclk(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_min_lclk(ryzen_access ry, uint32_t value){
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_prochot_deassertion_ramp(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_RAVEN:
    case FAM_PICASSO:
...
EXP int ApuDriver::set_apu_skin_temp_limit(ryzen_access ry, uint32_t value) {
    value *= 256;
    switch (ry->family)
    {
    case FAM_RENOIR:
...
EXP int ApuDriver::set_dgpu_skin_temp_limit(ryzen_access ry, uint32_t value) {
    value *= 256;
    switch (ry->family)
    {
    case FAM_RENOIR:
...
EXP int ApuDriver::set_apu_slow_limit(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_RENOIR:
    case FAM_LUCIENNE:
...
EXP int ApuDriver::set_skin_temp_power_limit(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_RENOIR:
    case FAM_LUCIENNE:
...
EXP int ApuDriver::set_gfx_clk(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_RENOIR:
    case FAM_LUCIENNE:
...
EXP int ApuDriver::set_power_saving(ryzen_access ry) {
    uint32_t value = 0;
    switch (ry->family)
    {
    case FAM_RAVEN:
...
EXP int ApuDriver::set_max_performance(ryzen_access ry) {
    uint32_t value = 0;
    switch (ry->family)
    {
    case FAM_RAVEN:
...
EXP int ApuDriver::set_oc_clk(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_LUCIENNE:
    case FAM_RENOIR:
...
EXP int ApuDriver::set_per_core_oc_clk(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_LUCIENNE:
    case FAM_RENOIR:
...
EXP int ApuDriver::set_oc_volt(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_LUCIENNE:
    case FAM_RENOIR:
...
EXP int ApuDriver::set_disable_oc(ryzen_access ry) {
    uint32_t value = 0x0;
    switch (ry->family)
    {
    case FAM_LUCIENNE:
...
EXP int ApuDriver::set_enable_oc(ryzen_access ry) {
    uint32_t value = 0x0;
    switch (ry->family)
    {
    case FAM_LUCIENNE:
...
EXP int ApuDriver::set_coall(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_CEZANNE:
    case FAM_RENOIR:
...
EXP int ApuDriver::set_coper(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_CEZANNE:
    case FAM_RENOIR:
...
EXP int ApuDriver::set_cogfx(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_CEZANNE:
    case FAM_RENOIR:
...
EXP int ApuDriver::set_gfx_clk_auto(ryzen_access ry) {
    uint32_t value = 0x0;
    switch (ry->family)
    {
    case FAM_RENOIR:
        _do_adjust_psmu(0x8A);
        break;
    }
    return ADJ_ERR_FAM_UNSUPPORTED;
}

EXP int ApuDriver::set_fclk_and_memclk(ryzen_access ry, uint32_t value) {
    switch (ry->family)
    {
    case FAM_RENOIR:
        _do_adjust_psmu(0x83);
        break;
    }
    return ADJ_ERR_FAM_UNSUPPORTED;
}

EXP int ApuDriver::set_fclk_and_memclk_auto(ryzen_access ry) {
    uint32_t value = 0x0;
    switch (ry->family)
    {
    case FAM_RENOIR:
        _do_adjust_psmu(0x84);
        break;
    }
    return ADJ_ERR_FAM_UNSUPPORTED;
}
...
EXP float ApuDriver::get_stapm_limit(ryzen_access ry){_read_float_value(0x0);}
EXP float ApuDriver::get_stapm_value(ryzen_access ry){_read_float_value(0x4);}
EXP float ApuDriver::get_fast_limit(ryzen_access ry){_read_float_value(0x8);}
EXP float ApuDriver::get_fast_value(ryzen_access ry){_read_float_value(0xC);}
EXP float ApuDriver::get_slow_limit(ryzen_access ry){_read_float_value(0x10);}
EXP float ApuDriver::get_slow_value(ryzen_access ry){_read_float_value(0x14);}

//custom section, offsets are depending on table version
EXP float ApuDriver::get_apu_slow_limit(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_apu_slow_value(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_vrm_current(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_vrm_current_value(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_vrmsoc_current(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_vrmsoc_current_value(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_vrmmax_current(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_vrmmax_current_value(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_vrmsocmax_current(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_vrmsocmax_current_value(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_tctl_temp(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_tctl_temp_value(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_apu_skin_temp_limit(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_apu_skin_temp_value(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_dgpu_skin_temp_limit(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_dgpu_skin_temp_value(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_psi0_current(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_psi0soc_current(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_cclk_setpoint(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_cclk_busy_value(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0001:
    case 0x001E0002:
...
EXP float ApuDriver::get_stapm_time(ryzen_access ry)
{
    switch (ry->table_ver)
    {
    case 0x001E0002:
...
EXP float ApuDriver::get_slow_time(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x001E0002:
...
EXP float ApuDriver::get_core_power(ryzen_access ry, uint32_t core) {
    switch (core)
    {
    case 0:
        switch (ry->table_ver)
...
EXP float ApuDriver::get_core_volt(ryzen_access ry, uint32_t core) {
    switch (core)
    {
    case 0:
        switch (ry->table_ver)
...
EXP float ApuDriver::get_core_temp(ryzen_access ry, uint32_t core) {
    switch (core)
    {
    case 0:
        switch (ry->table_ver)
...
EXP float ApuDriver::get_core_clk(ryzen_access ry, uint32_t core) {
    switch (core)
    {
    case 0:
        switch (ry->table_ver)
...
EXP float ApuDriver::get_l3_clk(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_l3_logic(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_l3_vddm(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_l3_temp(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_gfx_clk(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_gfx_volt(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_gfx_temp(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_fclk(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_mem_clk(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_soc_volt(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_soc_power(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
...
EXP float ApuDriver::get_socket_power(ryzen_access ry) {
    switch (ry->table_ver)
    {
    case 0x00370000:
    case 0x00370001:
